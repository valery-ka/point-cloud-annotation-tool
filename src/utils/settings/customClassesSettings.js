export const generateClassHotkeys = (existingHotkeys, configClasses) => {
    ...
    ["selectClass", "hideClass", "showClass"].forEach((key) => (newHotkeys[key] ??= {}));

    configClasses.forEach(({ originalIndex }, index) => {
        newHotkeys.selectClass[`selectClass${originalIndex}`] ??= (index + 1).toString();
        newHotkeys.hideClass[`hideClass${originalIndex}`] ??= `shift+${index + 1}`;
        newHotkeys.showClass[`showClass${originalIndex}`] ??= `alt+${index + 1}`;
    });
...
export const generatePointsSizes = (existingSizes, configClasses) => {
    ...
    configClasses.forEach((cls) => {
        const key = `${cls.label[0].toLowerCase() + cls.label.slice(1)}PointSize`;
...
export const getTranslatedCommand = (command, nonHiddenClasses, t) => {
    ...
    const classLabel =
        nonHiddenClasses.find((cls) => cls.originalIndex === originalIndex)?.label || originalIndex;

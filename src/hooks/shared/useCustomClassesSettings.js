...
export const useCustomClassesSettings = (settings, updateSettings) => {
    ...
    useEffect(() => {
        ...
        const allKeysAssigned = nonHiddenClasses.every(
            (cls, index) => hotkeys["selectClass"]?.[`selectClass${cls.originalIndex}`],
        );

        if (!allKeysAssigned) {
            const updatedHotkeys = generateClassHotkeys(hotkeys, nonHiddenClasses);
        ...
        const pointSizes = nonHiddenClasses.map((cls, idx) => {
            return `${cls.label[0].toLowerCase() + cls.label.slice(1)}PointSize`;
        });
        ...
        if (!allSizesAssigned) {
            const updatedPointSizes = generatePointsSizes(settingsSizes, nonHiddenClasses);
